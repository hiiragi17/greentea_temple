...
RSpec.describe "TempleDetails", type: :system do
  ...
  let!(:near_greentea1) { create(:greentea, latitude:34.9676, longitude:135.7741) }
  let!(:near_greentea2) { create(:greentea, latitude:34.9676, longitude:135.7741) }
  ...
  describe 'show temple' do
    context 'access temple#show' do
      ...
      it 'display near_greentea' do
        visit temple_path(temple)
        expect(Greentea.count).to eq(2)
