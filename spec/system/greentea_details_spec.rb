  ...
  describe 'show greentea' do
    context 'access greentea#show' do
      it 'display greentea_information' do
        ...
        # ビューは truncate(homepage, length: 32) で表示するため、リンクの href で検証する
        expect(page).to have_link(href: greentea.homepage), 'ホームページが表示されていません'
        expect(page).to have_content(greentea.holiday), '休日が表示されていません'
      end
      it 'display near_temple' do
        visit greentea_path(greentea)
        expect(all('.card').count).to eq(2)
        expect(page).to have_content(near_temple1.name), '近い神社仏閣が表示されていません'
        expect(page).to have_content(near_temple2.name), '近い神社仏閣が表示されていません'
        expect(page).not_to have_content(far_temple.name), '近くない神社仏閣が表示されています'
      end
      it 'show near_temple' do
        visit greentea_path(greentea)
        click_link near_temple1.name
        # Turbo の非同期遷移を待つため have_current_path（自動リトライ）で検証する
        expect(page).to have_current_path(temple_path(near_temple1))
      end
    end
  end
end
